...
// ============================================================
// ADMIN: HIGH COUNCIL MANAGEMENT
// ============================================================

function updateHighCouncilors(newNames, newUnits) {
  var config = getConfig();
  var oldNames = config.hc_list.split(",").map(function(n) { return n.trim(); });
  
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  
  // Update Approval column headers in the unified Records sheet
  var sheet = ss.getSheetByName("Records");
  if (sheet) {
    var values = sheet.getDataRange().getValues();
    if (values.length >= 1) {
